...
func main() {
	lambda.Start(listArticle)
	// LoadEnv(".env")
	// listArticle(context.Background(), events.APIGatewayProxyRequest{})
}
...
func listArticle(ctx context.Context, req events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	var (
	...
	logger.Printf("%s, \n", requestId)
	// if err != nil {
	// 	logger.Println("get list-article unmarshal body data err:", err)
	// 	resp.Body = "get list-article unmarshal body data err"
	// 	// return resp, nil
	// }

	lisst, err := getArticleDB(ctx)
	if err != nil {
		logger.Println("get list-article unmarshal body data err:", err)
		resp.Body = "get list-article from database err"
		resp.StatusCode = 400
		return resp, nil
	}
...
func getArticleDB(ctx context.Context) ([]ArticleDTO, error) {
	var (
		articles = []Article{}
		err      error
		resp     = []ArticleDTO{}
	...
	err = db.Debug().Table(Article{}.Table()).Find(&articles).Error
	if err != nil {
		return resp, err
	}
	// rolesModel, err := u.articleService.ListArticle(ctx, service.GetAll)
	// if err != nil {
	// 	log.Errorf("get list article, error while call database error %v", err)
	// 	return resp, common.ErrDatabase
	// }
	resp = make([]ArticleDTO, len(articles))
	// copier.Copy(&resp, &rolesModel)
	for i, item := range articles {
		image, _ := GetImageS3(ctx, GetImageS3Request{
			Data: GetImageS3RequestData{
